...
class Maze(abc.ABC):
    ...
    def plot_multiple_paths(self,paths, title_name=None):
        fig = plt.figure(1)
        ax1 = fig.add_subplot(1,1,1)
        ...
        for i,path in enumerate(paths):
            path = np.array(path)
            path_colors=[colors[i], 'k']
            for i in range(len(path)-1):
                cidx = i % 2
                plt.plot([path[i, 0], path[i+1, 0]], [path[i, 1], path[i+1, 1]],\
                         color=path_colors[cidx], linewidth=4)
        # legend_text=[]
        # for i in range(len(paths)):
        #     legend_text.append(str(i+1))
        # plt.legend(legend_text)
        colors=colors[0:len(paths)]
        legend_handles = [Line2D([0], [0], color=color, lw=2, label=f' {i + 1}') for i, color in enumerate(colors)]
    ...
    def check_hit(self, start, deltas):
        ...
        for i in range(int(norm)):
            ...
            if self.maze_array[int(x),int(y)] == 0:
                return True
            #making the crash detection more careful
            if self.maze_array[round(x),round(y)]==0:
                return True
